...
# Save dataset
write_xlsx(x = sum_levels, path = "results/data_analysis/3.sum_levels.xlsx")
```
...
ggsave(
  filename = "results/data_analysis/4.ggplot_contamination_sumfamily_taxalim.jpg",
...
ggdata_sumfamily_species <- contam |> 
  select(labels, grp, alim, sommePCB_ng_gdw, sommePFAS_ng_gdw, sommeHBCDD_ng_gdw) |> 
  pivot_longer(cols = c(sommePCB_ng_gdw, sommePFAS_ng_gdw, sommeHBCDD_ng_gdw), 
                names_to = "family", values_to = "sum_ng_gdw") |> 
  mutate(family = family |>  str_remove("somme"),
         family = family |>  str_remove("_ng_gdw"),
...
ggsave(
  filename = "results/data_analysis/5.ggplot_contamination_sumfamily_species.jpg",
  plot = ggplot_sumfamily_species, width = 20, height = 10, units = "cm"
)
```

# 6. Total levels in PCBs and HBCDD isomers versur lipid content
...
There is a weak but significant negative correlation between lipid content and contamination levels of HBCDD isomers (Spearman's test: p-value = 0.01, ρ = -0.38). Below 4% lipid content (dw), contamination levels appear particularly variable, with notable differences among species.
```{r}
cor.test(x = contam$lipid_percent_dw, y = contam$sommeHBCDD_ng_gdw, method = "spearman")
```

```{r}
ggplot_lipid_HBCDD <- ggplot(contam) +
  aes(x = lipid_percent_dw, y = sommeHBCDD_ng_gdw, color = labels) +
  geom_point() +
  scale_color_manual(values = my_colors) +
  labs(title = "HBCDD", x = "Lipid content (%dw)", y = "Sum concentrations (ng/g dw)", color = NULL)
ggplot_lipid_HBCDD
```

```{r}
ggplot_lipid <- ggplot_lipid_PCB + ggplot_lipid_HBCDD
ggsave("results/data_analysis/6.lipid_vs_contaminants.png", ggplot_lipid, width = 10, height = 5, dpi = 300)
```
...
contam_profile <- contam |> 
  ...
  mutate(chemical = chemical |> str_remove("_normalised_sum_ng_gdw"),
         family = case_when(str_starts(chemical, "CB") ~ "PCB",
                            str_detect(chemical, "HBCDD") ~ "HBCDD",
                            TRUE ~ "PFAS")) |> 
  mutate(family = factor(family, levels = c("PCB", "PFAS", "HBCDD")),
...
ggsave("results/data_analysis/7.contamination_profiles.jpg", ggplot_profile, width = 8, height = 4, dpi = 300)
```
...
writexl::write_xlsx(x = contam_profile_report, path = "results/data_analysis/7.contamination_profile.xlsx")
```
...
ggplot_contam_profile_prop_PCB <- ggplot(contam_profile_prop_PCB) +
  ...
  labs(y = "Percentage of total concentrations (ng/g dw)", 
       size = "Chlorine substitution", color = "Chlorine substitution")
ggplot_contam_profile_prop_PCB

ggsave("results/data_analysis/8.contam_profiles_prop_PCB.jpg",
       ggplot_contam_profile_prop_PCB, width = 6, height = 4, dpi = 300)
```
...
ggplot_contam_profile_prop_PFAS <- ggplot(contam_profile_prop_PFAS) +
  ...
  geom_point() +
  geom_text_repel(data = points_to_label_bottom,
                  aes(label = chemical), size = 3,
                  point.padding = 1, max.overlaps = 20) +
    geom_text_repel(data = points_to_label_up,
                  aes(label = chemical), size = 3,
                  point.padding = 1, max.overlaps = 20) +
  labs(y = "Percentage of total concentrations (ng/g dw)",
...
ggsave("results/data_analysis/8.contam_profiles_prop_PFAS.jpg",
